...
function QuizTakingPage() {
  ...
  const onNext = () => setIndex((i) => Math.min(i + 1, Math.max(0, words.length - 1)))
  const onPrev = () => setIndex((i) => Math.max(0, i - 1))
  const onSubmit = () => {
    const payload: SubmitAnswer[] = words.map((qw) => ({ wordId: qw.wordId, answer: answers[qw.wordId] ?? "" }))
                ...
                <div className="space-y-3">
                  ...
                  <div className="border rounded-md p-4 space-y-2">
                    <div className="text-2xl font-semibold">
                      {words[index].word.hiragana || words[index].word.katakana || words[index].word.kanji}
                    </div>
                    {words[index].word.pronunciation && (
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => new Audio(words[index].word.pronunciation).play().catch(() => {})}
                  ...
                  <div className="flex items-center justify-between">
                    <Button variant="secondary" onClick={onPrev} disabled={index === 0}>Previous</Button>
