...
import type { WordsListResponse, CreateQuizPayload, QuizDetail } from "@/types/api";
...
function QuizCreationPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (quiz) => {
      ...
      router.navigate({ to: "/quizzes/$id", params: { id: quiz.id } })
    },
  ...
  const onSubmit = () => {
    const wordIds = Object.entries(selected).filter(([, v]) => v).map(([k]) => k)
          ...
          <CardContent className="space-y-3">
            <div className="grid sm:grid-cols-2 gap-3">
              <div>
                ...
                <Input id="title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="My N5 Set" />
              </div>
              <div>
                ...
                <div className="max-h-80 overflow-auto border rounded-md p-3 grid sm:grid-cols-2 md:grid-cols-3 gap-2">
                  {data?.words.map((w) => (
                    <label key={w.id} className="flex items-center gap-2 cursor-pointer">
                      ...
                      <span className="text-sm">{w.hiragana || w.katakana || w.kanji} — {w.meaning}</span>
                    </label>
                  ))}
                </div>
              )}
